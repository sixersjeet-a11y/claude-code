...
- Foreign keys where needed, including optional school relations for interviews/essays/documents
- Query indexes for performance (`state`, `competitiveness`, composite school filters, user timelines, due dates, etc.)
- Import audit table (`SchoolImportJob`) to track CSV ingestion quality and volume over time

## 4) API Endpoints

### Core
- `POST /api/import-schools` upload CSV and ingest schools.
- `GET /api/schools` school search endpoint with pagination and advanced filters (`state`, `boarding`, `competitiveness`, `religiousAffiliation`, `sizeMin`, `sizeMax`).
...
4. Normalize values (boolean, numeric, acceptance rate) and insert validated schools.
5. Log each import into `SchoolImportJob` with processed/inserted/skipped totals.
6. Support future upload through API or CLI script.
...
## Performance
- Database composite indexes for multi-filter explorer queries and school-linked activity timelines.
